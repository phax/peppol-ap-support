...
{
  private static final Logger LOGGER = LoggerFactory.getLogger (PeppolReportFlywayMigrator.Singleton.class);
                  ...
                  @Nonnull final FlywayConfiguration aFlywayConfig)
  {
    ...
    final FluentConfiguration aActualFlywayConfig = Flyway.configure ()
                                                          .dataSource (new DriverDataSource (PeppolReportFlywayMigrator.class.getClassLoader (),
    ...
    aActualFlywayConfig.baselineVersion (Integer.toString (aFlywayConfig.getFlywayBaselineVersion ()))
                       .baselineDescription ("Peppol Report Storage Baseline");

    // Separate directory per DB type
    aActualFlywayConfig.locations ("db/peppol-report-" + eDBType.getID ());
